...
TEST(ArgParserTestSuite, MyOwnTest) {
    ArgParser parser("My Parser");
    parser.AddIntArgument('i', "int", "Some Description about program").Positional();

    ASSERT_TRUE(parser.Parse(SplitString("app -34")));
    ASSERT_EQ(parser.GetIntValue("int"), -34);
}

TEST(ArgParserTestSuite, HelpStringTest) {
    ArgParser parser("My Parser");
    parser.AddHelp('h', "help", "Some Description about program");
    parser.AddStringArgument('i', "input", "File path for input file").MultiValue(1).Default("default value");
    parser.AddFlag('s', "flag1", "Use some logic").Default(true);
    parser.AddFlag('p', "flag2", "Use some logic").Positional().Default(true);
    parser.AddIntArgument("numer", "Some Number").Default(0);
